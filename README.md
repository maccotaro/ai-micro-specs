...
### 🖥️ フロントエンドサービス
- **[04-user-frontend/](./04-user-frontend/)** - ユーザーフロントエンド（ai-micro-front-user）
  - 画面設計書
  - BFF API連携設計書
  - コンポーネント設計書

- **[05-admin-frontend/](./05-admin-frontend/)** - 管理フロントエンド（ai-micro-front-admin）
  - 画面設計書
  - ドキュメント管理機能設計書
  - OCR結果表示UI設計書

### 🗄️ インフラサービス
- **[06-database/](./06-database/)** - PostgreSQLデータベース（ai-micro-postgres）
  - データベース構成設計書
  - スキーマ設計書（authdb/apidb/admindb）
  - ER図
  - マイグレーション管理

- **[07-redis/](./07-redis/)** - Redisキャッシュ（ai-micro-redis）
  - データ構造設計書
  - キャッシュ戦略設計書
  - セッション管理設計書
...
- **[11-operations/](./11-operations/)** - 運用ガイド
  - システム起動・停止手順
  - 監視設計書
  - トラブルシューティング
  - バックアップ・リストア
...
### 🛠️ 開発・運用
- **[12-development/](./12-development/)** - 開発ガイド
  - 開発環境セットアップ
  - コーディング規約
  - Gitワークフロー
  - テストガイド

- **[13-deployment/](./13-deployment/)** - デプロイガイド
  - デプロイ概要
  - Docker Compose設定
  - 環境変数一覧
  - CI/CDパイプライン

- **[14-performance/](./14-performance/)** - パフォーマンス設計
  - パフォーマンス全体方針
  - 負荷テスト
  - 最適化ガイド
  - スケーラビリティ設計
...
### 📝 その他
- **[15-testing/](./15-testing/)** - テスト設計
  - テスト戦略
  - ユニットテスト・統合テスト
  - E2Eテスト
  - 契約テスト

- **[16-changelog/](./16-changelog/)** - 変更履歴
  - システム変更履歴
  - 重要な修正の記録
...
3. `08-integration/02-authentication-flow.md` - 認証フローを確認
4. `09-api-contracts/` - API仕様を確認
5. 開発対象サービスのドキュメント

### 🎨 フロントエンド開発者
1. **システム全体**（上記の初めて読む方と同じ）
2. `04-user-frontend/03-api-integration.md` または `05-admin-frontend/03-api-integration.md` - BFFパターンを理解
3. `09-api-contracts/` - API仕様を確認
4. `08-integration/02-authentication-flow.md` - 認証フローを確認
5. フロントエンド詳細ドキュメント

### 🔧 インフラ担当者
1. `00-overview/03-infrastructure.md` - インフラ全体像
2. `06-database/` - データベース設計
3. `07-redis/` - キャッシュ設計
4. `11-operations/` - 運用手順
5. `13-deployment/` - デプロイ手順

### 🛡️ セキュリティ担当者
1. `10-security/01-security-overview.md` - セキュリティ全体方針
2. `01-auth-service/03-jwt-design.md` - JWT設計
3. `08-integration/02-authentication-flow.md` - 認証フロー
4. `10-security/` 配下のすべてのドキュメント
